...
│   ├── symplectic.py      # shared S/d (G+B gates)
│   ├── gaussian/          # (V, r̄) + gates; symplectic re-export shim
│   ├── fock/              # truncated amplitudes; independent of G/B
│   ├── bosonic/           # components + cat; gates → cvsim.symplectic
│   ├── wigner.py          # cross-rep facade (intentional)
...
| `cvsim/symplectic.py` | Shared xxpp S/d generators (only place for BS/D/R/S formulas) |
| `cvsim/gaussian/symplectic.py` | Compat re-export of `cvsim.symplectic` |
| `cvsim/gaussian/` | GaussianState, gates D/R/S/BS/S₂, `channels.loss`, det/⟨n⟩, Homodyne mean/var/**condition** |
| `cvsim/fock/` | FockState 1–2 mode, D/R/S/Kerr, BS, norm/⟨n⟩/pnrd_probs; no G/B import |
| `cvsim/bosonic/` | Components, cat, gates import `cvsim.symplectic` (not gaussian package), weighted ⟨n⟩/Homodyne |
...
New gate matrices go in `cvsim/symplectic.py` first; backends only apply maps. B must not import `cvsim.gaussian.*`.
...
- B1: `cvsim/symplectic.py` + `tests/test_b1_*.py`
