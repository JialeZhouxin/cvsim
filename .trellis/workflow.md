...
Grill pass: when requirements have soft spots, use the `grilling` interview (one question at a time, recommended answer each); persist resolved terms to `CONTEXT.md` via `domain-modeling`, ADRs to `docs/adr/` when hard-to-reverse + surprising + real trade-off.
[/workflow-state:planning]
...
**Grill pass (grill-with-docs).** When a decision area still has soft spots after the brainstorm pass, switch to the `grilling` interview: walk the decision tree one branch at a time, one question per message, each with a recommended answer and the trade-off of choosing differently. Facts the repo can answer are looked up, never asked.

**Domain-modeling (terminology + ADRs).** While exploring, capture the project's ubiquitous language:
- On the first resolved term, create/update `CONTEXT.md` at the repo root (glossary only — no implementation details; format per `domain-modeling`/CONTEXT-FORMAT.md). Every subsequent task uses its vocabulary; call out term conflicts when the user's language drifts from the glossary.
- Offer an ADR under `docs/adr/` only when all three hold: hard to reverse · surprising without context · a real trade-off. Otherwise skip; ADR-FORMAT.md defines the shape.
- The `grill-with-docs` skill is the wrapper for grill + glossary + ADR in one flow.
