...
const PORT = port(8765, "PROBE_PORT"), CDP_PORT = port(9224, "PROBE_CDP_PORT");
const server = spawn(uvicornPath(), ["cvsim.lab.server:app", "--port", String(PORT), "--log-level", "warning"], { stdio: "ignore" });
const edge = spawn(EDGE, [
  "--headless=new", "--disable-gpu", "--no-first-run", "--no-default-browser-check",
  `--remote-debugging-port=${CDP_PORT}`, "--user-data-dir=" + userDataDir("edge-debug-probe"),
