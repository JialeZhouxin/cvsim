...
def test_assemble_schema_top_level_keys():
    ...
    assert set(payload) == {"backends", "ops", "initial", "meters", "extensions"}
...
def test_meters_matrix_embedded():
    """Meter support matrix (ADR-0008): embedded by reference from lab.result.

    Core trio + per-representation extensions, sorted for stable JSON. Key
    sets are NOT aligned (physical differences stay honest, no fabricated
    None); the frontend consumes this in a follow-up ticket (R6).
    """
    payload = assemble_schema()
    assert payload["meters"]["core"] == [
        "mean_photon",
    ...
    # no fabricated alignment: extension keys must not leak into other rows
    assert "leakage" not in payload["meters"]["extensions"]["gaussian"]
...
def test_422_bosonic_initial_semantics_text_unchanged():
    ...
    msg = _detail_for(
        {"schema": "circuit_v1", "nmode": 2, "backend": "bosonic", "initial": [0, 3], "ops": []}
    )
...
def test_422_bosonic_generic_initial_message_unchanged():
    ...
    assert msg == ("initial must be a list of null/'gkp0'/'gkp1'/'gkp0_2d'/'gkp1_2d' per mode")


def test_422_fock_initial_message_unchanged():
    """Core fock IR validation fires first (stricter/equal to the Lab check)."""
    msg = _detail_for(
        {"schema": "circuit_v1", "nmode": 2, "backend": "fock", "initial": [1.5, 0], "ops": []}
    )
    assert msg == "initial must be a list of nmode=2 ints, got [1.5, 0]"
    msg2 = _detail_for(
        {"schema": "circuit_v1", "nmode": 2, "backend": "fock", "initial": [-1, 0], "ops": []}
    )
...
def test_422_whitelist_byte_identical_node_id():
    ...
    body = {
        ...
        "backend": "fock",
        "ops": [
            {"id": "x", "op": "interferometer", "modes": [0, 1], "params": {"U": [[1, 0], [0, 1]]}}
        ],
    }
