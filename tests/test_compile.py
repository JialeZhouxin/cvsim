...
def _naive_apply(op_name, st, modes, **kwargs):
    ...
    if op_name == 'beamsplitter':
        return beamsplitter(
            st, modes[0], modes[1], kwargs['theta'], kwargs.get('phi', 0.0)
        )
    ...
    if op_name == 'interferometer':
        return apply_symplectic(
            st, _naive_interferometer_s(st.nmode, kwargs['U']), validate=False
        )
    if op_name == 'loss':
    ...
    if op_name == 'amplifier':
        ...
        return amplifier(st, kwargs['G'], modes[0] if modes else None,
                         kwargs.get('nbar', 0.0))
    ...
    for op_name, modes, fixed, pnames, refs in circ._ops:
        ...
        if op_name == 'measure_homodyne':
            ...
            st = apply_gaussian_channel(
                st, kwargs['X'], kwargs['Y'], kwargs.get('d'),
        ...
        while n_naive < target:
            ...
            if op_name == 'measure_homodyne':
                ...
                st_n = apply_gaussian_channel(
                    st_n, kwargs['X'], kwargs['Y'], kwargs.get('d'),
                    validate=kwargs.get('validate', True),
                )
            else:
