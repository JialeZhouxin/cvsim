...
def _loss_XY(nmode: int, T: float, mode: int | None) -> tuple[np.ndarray, np.ndarray]:
    """X, Y same as Gaussian loss (ħ=1, V_vac=I/2)."""
...
def loss(
    state: BosonicState, T: float, mode: int | None = None
) -> BosonicState:
