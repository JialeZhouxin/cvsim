...
**Applies to:** public surface of `cvsim.gaussian`, `cvsim.symplectic`, `cvsim.conventions`, `cvsim.bridge`  
**Date:** 2026-07-30
...
| `cvsim.bosonic` | **B1 frozen surface** (A11, B1 exit): `__all__` = `BOSONIC_PUBLIC` freeze in `test_public_api.py`; `gkp_logical_overlap` deprecated (teaching, diagonal-peak); B2+ additions go through this table. **B3+ addition (ADR-0007)**: `heterodyne_pdf` (exact 2D Husimi Q surface) added to `__all__`; `heterodyne_sample` output distribution now exact — same-seed stream vs Gaussian package retired (statistical equivalence guaranteed instead); `imag_tol` parameter removed from heterodyne entries (teaching-cut pool deleted). **B9 addition**: `pnr_probs` and `pnr_sample` are public single-mode Bosonic APIs; `cutoff` truncates returned probabilities without renormalization, while sampling renormalizes within cutoff. **B10 addition (joint PNR)**: `pnr_probs`/`pnr_sample` accept a `modes` parameter — `modes=None` gives the joint distribution over all modes, `modes=(j,…)` over a subset (returns a tensor of shape `(cutoff,)*k` / `tuple[int,…]`), mutually exclusive with `mode=`; joint path is the vectorized complex-centre Cauchy kernel (branch-anchored). `pnr_condition` landed via the root state bridge — see the `cvsim.bridge` row. |
| `cvsim.bridge` | Root-level bridge (ADR-0001/ADR-0012; rep packages cannot import it). **Element golds**: `coherent_element` / `squeezed_element` / `thermal_diag` / `vacuum_probability` / `fock_state_amplitude`. **State bridge (ADR-0012)**: `bosonic_to_fock(state, *, cutoff) -> FockDensity`, `pnr_condition_bosonic(state, mode, n, *, cutoff) -> FockDensity`, `pnr_sample_and_condition_bosonic(state, mode, *, cutoff, rng=None) -> (n, FockDensity)` — one-way bosonic→fock, **no renormalization** (trace = 1 − tail); honest `ValueError` boundaries: nmode ≤ 2, no 2-mode mixed components, no complex-r̄×mixed.
